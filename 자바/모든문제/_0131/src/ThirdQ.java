    ...
    public int[] solution(int[] fees, String[] records) {
        ...
        for (int i = 0; i < records.length; i++) {
            ...
            if (map.containsKey(infos[1])) {
                String[] intTime = map.remove(infos[1]).split(":");
                String[] outTime = infos[0].split(":");

                int hour = Integer.parseInt(outTime[0]) - Integer.parseInt(intTime[0]);
                int minute = Integer.parseInt(outTime[1]) - Integer.parseInt(intTime[1]);
