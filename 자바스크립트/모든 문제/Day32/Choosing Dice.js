function solution(dices) {
  ...
  for (let i = 0; i < groups.length; i += 1) {
    ...
    const { countMap: groupDupCountMap, unique: groupUnique } =
      getDupCountMapAndUnique(groupBucket);

    const { countMap: oppoGroupDupCountMap } =
      getDupCountMapAndUnique(oppoGroupBucket);
    ...
    for (const dice of groupUnique) {
      ...
      const pin1 = binaryMinIdxSearch(
        0,
        oppoGroupBucket.length - 1,
        oppoGroupBucket,
        dice
      );
